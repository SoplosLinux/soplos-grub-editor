...
class GrubManager:
    ...
    def set_debian_theme_active(self, active: bool) -> bool:
        """
        Enable/Disable the default Debian theme script.
        This prevents conflicts with our custom settings.
        """
        ...
        try:
            ...
            log_info(_("Set 05_debian_theme active={active}").format(active=active))
            return True
        except Exception as e:
            log_error(_("Failed to change 05_debian_theme permissions: {err}").format(err=e))
            return False
    ...
    def save_custom_ui_settings(self, bg_path: str, color_normal: str, color_highlight: str) -> bool:
        ...
        clean_config = {
            'GRUB_THEME': '',
            'GRUB_BACKGROUND': '',        # Remove from here, moving to custom.cfg
            'GRUB_COLOR_NORMAL': '',      # Remove from here
            'GRUB_COLOR_HIGHLIGHT': '',   # Remove from here
        }
        ...
        if bg_path:
            content.append(f"background_image '{bg_path}'")
        ...
        # 3. Disable Debian theme script to prevent override
        self.set_debian_theme_active(False)
